...
for ax, model, name in zip(axes, models, names):
    ...
    cf = ax.pcolormesh(xx, yy, Z_probs, cmap='RdBu_r', 
                       vmin=0, vmax=1, shading='auto', alpha=0.9)
    
    # B. Add Contour Line for Decision Boundary
    ax.contour(xx, yy, Z_probs, levels=[0.5], colors='black', linewidths=2, linestyles='--')
    ...
    ax.scatter(X[mask_slice & (y==0), 0], X[mask_slice & (y==0), 1], 
               c='darkred', edgecolors='w', s=30, label='Class 0')
    ax.scatter(X[mask_slice & (y==1), 0], X[mask_slice & (y==1), 1], 
               c='navy', edgecolors='w', s=30, label='Class 1')
